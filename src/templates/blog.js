...
export default function Blog({ data, context, isGrid }) {
  ...
  for (let i = 0; i < posts.length; i++) {
    postArray.push([
      posts[i].fields.slug,
      posts[i].frontmatter.featuredImage.childImageSharp.fluid.src,
      posts[i].frontmatter.title,
      posts[i].frontmatter.date,
      posts[i].excerpt,
    ])
  }
  let pageNumArray = []
    ...
    <div>
      ...
      <div className="pageNumMenu">
        {pageNumArray.map(pageNum => (
          <Link
            className="pageNum"
            to={pageNum === 1 ? "/" : "/page" + pageNum.toString()}
          >
            {pageNum === context.currentPage ? (
              <div className="currentPageLink">{pageNum}</div>
            ) : (
              <div className="otherPageLink">{pageNum}</div>
            )}
          </Link>
        ))}
      </div>
